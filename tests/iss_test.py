...
def _truncate_to_one_minute(seconds: int) -> int:
    s = seconds // 60 * 60
    hmsz = f"{dt.datetime.fromtimestamp(s,dt.UTC)}".split()[1]
...
class IssTest(unittest.TestCase):
    ...
    def test_timezone_roundtrip(self) -> None:
        ...
        stamp1 = fromtimestamp(august, dt.UTC)
        ...
        self.assertEqual("2024-08-31T17:22:00+00:00", fromtimestamp(seconds, dt.UTC).isoformat())
        self.assertEqual(dt.UTC, stamp1.tzinfo)
